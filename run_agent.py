...
class MCPAgent:
    ...
    def __init__(
        self, server_script: str = "mcp_server.py", data_dir: Path = Path("data")
    ):
    ...
    async def connect(self):
        ...
        # Connect as client using context manager properly
        self.stdio_client_context = stdio_client(server_params)
        read_stream, write_stream = await self.stdio_client_context.__aenter__()
    ...
    async def disconnect(self):
        ...
        if hasattr(self, "stdio_client_context") and self.stdio_client_context:
    ...
    async def process_request(self, request_id: str) -> Decision:
        ...
        if not validation_result.get("ok", False):
            ...
            customer_message = self.generate_error_message(
    ...
    def generate_clarifying_question(self, validation):
        ...
        if "texture channels" in errors.lower():
            return (
                "Should we use default channel mappings or wait for your configuration?"
            )
            ...
            try:
                ...
                response = await self.llm_client.chat.completions.create(
                    ...
                    messages=[
                        {
                            "role": "system",
                            "content": "You are helping explain 3D asset processing decisions to customers.",
                        },
                        {
                            "role": "user",
                            "content": f"Explain this validation failure in a friendly way: {decision.validation_result}",
                        },
                    ],
                    temperature=0.7,
                    max_tokens=200,
...
async def main():
    ...
    # Create data directory path from the first file's directory
    data_dir = Path(args.requests).parent

    # Start MCP server
    server_params = StdioServerParameters(
        command="/Users/bryantan/Documents/GitHub/Kaedim_MCP_Agent/.venv/bin/python",
        args=[args.server, str(data_dir)],
        env=None,
    )
    ...
    async with stdio_client(server_params) as (read_stream, write_stream):
        async with ClientSession(read_stream, write_stream) as session:
            ...
            logger.info(
                f"Available resources: {[resource.uri for resource in resources.resources]}"
            )

            # Create agent with session
            agent = SimpleMCPAgent(session)
            ...
            print(
                f"Individual file arguments: requests={args.requests}, artists={args.artists}, presets={args.presets}, rules={args.rules}"
            )
            print(f"Data directory inferred: {data_dir}")


class SimpleMCPAgent:
    """Simplified agent that uses an existing MCP session"""
